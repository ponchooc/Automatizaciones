...
def consultar_lotes_ordenes(lista_datos_entrada, sesion=None, sesion_otm=None):
    ...
    try:
        ...
        if respuesta.status_code == 200:
            ...
            # un estatus 404 de oracle wms representa una respuesta de negocio valida que indica que la orden no existe
            pass
        else:
            # cualquier otro estatus que no sea 200 ni 404 si representa un fallo real del servicio de red
            wms_api_fallo_real = True
        ...
        for carlos, sales, bodega_orig in lista_datos_entrada:
            ...
            # ®Carlos Alfonso Ortega Molina®
            # normalizacion flexible de cadenas con y sin ceros a la izquierda para evitar falsos negativos en el cruce de datos
            carlos_sin_cero = carlos.lstrip('0')
            existe_wms = (carlos in encontrados) or (carlos_sin_cero in encontrados)
            existe_otm = (carlos in encontrados_otm) or (carlos_sin_cero in encontrados_otm)
            # ®Carlos Alfonso Ortega Molina®
            
            if otm_error: existe_otm = False
            
            if existe_wms and existe_otm:
                clave_wms = carlos if carlos in encontrados else carlos_sin_cero
                info = encontrados[clave_wms]
                res_obj.update({
