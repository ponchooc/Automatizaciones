...
def subir_ftp_y_ejecutar_ssh(errores_pendientes):
    ...
    try:
        # usamos newlinen y encoding latin1 para compatibilidad con gnx
    ...
    try:
        ...
        sh.send("exit\n")
        time.sleep(3) # pausa rigurosa para que el sistema procese la salida y muestre el menu
...
def consultar_lotes_ordenes(lista_datos_entrada, sesion, sesion_otm=None):
    mapa_lote = {carlos: sales for carlos, sales in lista_datos_entrada}
    ordenes_str = ",".join(mapa_lote.keys())
    resultados_finales_lote = []
    
    try:
        respuesta = sesion.get(URL_BASE, params={'order_nbr__in': ordenes_str, 'page_size': 100}, timeout=25)
        ...
        if sesion_otm:
            ...
            otm_params = {'q': q_param, 'fields': 'orderReleaseXid', 'limit': 100}
            try:
...
def enviar_correo(total_gnx, total_hoy, validados, lista_pendientes, datos_todos=None):
    ...
    fuente = datos_todos if datos_todos else lista_pendientes
    cnt_solo_otm = sum(1 for r in fuente if "existe en wms pero no otm" in r.get('resultado', ''))
    cnt_solo_wms = sum(1 for r in fuente if "existe en otm pero no wms" in r.get('resultado', ''))
    cnt_ambos    = sum(1 for r in fuente if "no existe en wms ni otm"  in r.get('resultado', ''))
...
def ejecutar_verificacion():
    ...
    if para_api:
        print(f"\n ════════════════════════════════════════════════════════════")
        print(f"  [FASE 1] VALIDACION BATCH WMS & OTM (Lotes de 100)")
        ...
        with requests.Session() as sesion, requests.Session() as sesion_otm:
             ...
             with ThreadPoolExecutor(max_workers=5) as ex:
                 futuros = {ex.submit(consultar_lotes_ordenes, l, sesion, sesion_otm): i for i, l in enumerate(lotes)}
                 idx = 1
